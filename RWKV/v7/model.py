...
class RWKV(nn.Module):
    ...
    def forward(self, idx, states: BlockStateList = None):
        args = self.args
        B, T = idx.size()
    ...
    def train_forward(self, idx, v_first, last_shift_states: torch.Tensor, last_wkv_states: torch.Tensor):
        args = self.args
        B, T = idx.size()
        assert T <= args.ctx_len, "Cannot forward, model ctx_len is exhausted."
        C = args.n_embd
    ...
    def train_step(self, idx, targets, states: BlockStateList = None):
        args = self.args
        ...
        def checkpointed_step(idx, targets, prev_loss, v_first, last_shift_states, last_wkv_states, prev_token_amount):
            ...
            logits, v_first, new_shift_states, new_wkv_states = self.train_forward(idx, v_first, last_shift_states, last_wkv_states)

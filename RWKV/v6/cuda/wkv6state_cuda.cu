...
__global__ void kernel_forward(const int B, const int T, const int C, const int H,
                               ...
                               F *__restrict__ const _y, F *__restrict__  _s_out)
{
...
void cuda_forward(int B, int T, int C, int H, bf16 *r, bf16 *k, bf16 *v, bf16 *w, bf16 *u, bf16 *z, bf16 *y, bf16 *s_out) {
    assert(H*_N_ == C);
    assert(_N_%4 == 0);

    // Launch kernel with appropriate grid and block dimensions
    kernel_forward<<<dim3(B * H), dim3(_N_)>>>(B, T, C, H, r, k, v, w, u, z, y, s_out);
